...
public class MyBot9 : IChessBot
{
  ...
  public int EvalMove(Timer? timer, Board board, int depth, int alpha, int beta, out Move bestMove)
  {
    ...
    var captureMoves = legalMoves.Where(m => m.IsCapture && m.CapturePieceType != PieceType.Pawn);

    if (depth <= 0 && !captureMoves.Any() && !board.IsInCheck())
    {
    ...
    var allMoves = goodMoves.Concat(promotionMoves).Concat(captureMoves).ToList();
